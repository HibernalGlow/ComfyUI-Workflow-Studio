...
Edit サブタブMVP実装後、実際にユーザーが使う中で見つかった調整要望をまとめて対応した。

**プレビュー枠の高さ固定**: 中央ペインのSource/Resultプレビュー枠が`min-height`指定のみだったため、動画ソースのネイティブサイズ（特に縦長）によって枠が伸縮し、下のPlan/Editサブタブの表示位置がソースを選ぶたびに動いてしまっていた。枠を`height:260px`固定にし、動画/画像側は`width/height:auto; max-width/max-height:100%`（ネイティブサイズが枠より大きい場合のみ縮小、小さい場合は拡大しない）に変更して解消。

**AssetサブタブをPlan/Editと並ぶ中央下部の第3サブタブへ移設**: 左サイドバーにあったAsset一覧・詳細パネルを中央ペイン下部（Plan/Editと同じ場所）へ移設し、左=一覧（サムネイル/テーブル切替可）・右=選択ファイルの詳細という2カラムレイアウトに再構成。一覧上部にグループ切替・ファイル名検索・表示切替（▦グリッド/☰テーブル）ボタンを追加した。左サイドバーはProjectのみのシンプルなパネルになった。

**トリムパネルにClipchamp風ビジュアルスクラバーを追加**: 動画クリップのトリム編集がテキスト入力（開始/終了秒）のみだったのを、時間目盛り（ルーラー）・Sourceプレビューの`currentTime`と同期するドラッグ可能な再生ヘッド・時刻バッジ付きのドラッグ可能なイン/アウトハンドルを備えたビジュアルなスクラバーに拡張した。全位置を`clip.duration`基準の%指定にすることでpx/秒換算の管理を持たずに済ませている。既存のStart/End数値入力・「現在位置を使用」ボタンとは同じ`commit`関数を通して連動する。ハンドルドラッグ中は`_renderTimeline()`の再描画を避け、pointerup時のみ確定してパフォーマンスに配慮。playheadとトリムハンドルが同じ位置に重なってもハンドル操作を優先できるようz-indexを調整した。

**Frame保存のVideo Assets登録とAsset→Plan画像連携**: 右ペインFrameタブの「Save to Output」に「Add to Video Assets group」チェックボックスを追加し、チェック時は保存したフレームをGalleryの`__Video Assets__`グループへ自動登録できるようにした。またAssetタブの画像アセット詳細に「Set as First Image」「Set as Last Image」ボタンを追加し、PlanタブのFirst/Last Imageへ直接画像をアップロードできるようにした（動画クリップでは非表示、選択中のPlanブロックへ反映後Planサブタブへ自動切り替え）。

**検証**: いずれもKaptureで実機ComfyUI_5（ポート8189）に接続し、正方形/縦長動画でのプレビュー枠固定確認、Asset新レイアウトでのグリッド/テーブル切替・検索・選択、トリムスクラバーのplayhead同期・トラッククリックシーク・ハンドルドラッグ（playhead重なり時含む）・複数クリップ切替時のリスナー管理、Frame保存後のGalleryグループ登録をAPI経由で確認、Asset画像からFirst/Last Imageへの反映まで一通り実機確認済み。

**How to apply**: プレビュー枠のようにコンテンツ依存でサイズが変わる要素が、隣接するUIの表示位置に影響する場合は、コンテンツ側ではなく枠側を固定サイズにしてコンテンツを内側で縮小表示する方が安全（[[project_video_edit_tab_feature_branch]]）。ドラッグ可能なUI要素同士が同じ座標に重なる可能性がある場合は、操作頻度の高い方（今回はトリムハンドル）のz-indexを優先し、もう一方の当たり判定に埋もれないようにする。
...
ユーザーから「ギャラリータブで検索を行うと読み込み中のままでクリアを行っても中断できない。50枚程度のフォルダで検索をかけても時間がかかっている」という報告を受け、原因調査から修正まで対応した。

**フロントエンドの2つの不具合**（`gallery-tab.js`）: 検索欄はキー入力のたびに即座に（debounceなしで）`loadImages()`を呼んでおり、しかも直前のfetchをキャンセルする仕組み（AbortController）が無かった。そのため「クリア」を押した直後でも、その前に打っていた検索文字列の（まだ処理中の）リクエストが後から返ってきて、クリア後の空検索の結果を上書きしてしまい、「読み込み中のまま/検索結果のまま」に見えていた。検索入力に300msのdebounceを追加し、`loadImages()`にAbortController管理を追加（新しいリクエスト発行時に前回の未完了リクエストを中断）、「クリア」「Clear filters」ボタンはdebounceタイマーもクリアして即座に反映するようにした。

**バックエンドのロック競合**（`gallery_metadata.py`, `gallery_service.py`）: フォルダを開くたびに起動するバックグラウンドインデックス処理が、`gallery_metadata.json`（画像数が多い環境では1MBを超える）全体を`threading.RLock`保持したままJSONシリアライズ+ディスク書き込みしており、その間ずっと検索リクエストの`get()`呼び出しがロック待ちでブロックされていた。さらに10枚処理するごとに1件ずつ`save()`を呼んでいたため、実質10回分の全体書き込みが発生していた。`_save_to_disk()`をロック内ではデータのdeepcopyのみに留め、実際のファイルI/Oは別の書き込み専用ロック（`_write_lock`）でロック外に出すよう変更し、バックグラウンドインデックス側もまとめて1回で保存する`save_batch()`を新設して使うようにした。また検索時は（該当時に全メタデータのJSON全体書き込みを伴う）`cleanup_stale_images()`をスキップするようにした（通常の検索なしロード時に実行されれば十分なため）。

**根本原因: ponyxlWildcardsVaultフォールバック探索でのVaultフォルダ誤検出**（`gallery_service.py`）: 上記2件を直しても実機で53枚のフォルダへの検索が約3秒かかっており、一時的な計測ログを実機に仕込んで原因を特定した。出力ルート直下にComfyUI/StabilityMatrixが作る空の`thumbnails`フォルダがたまたま存在しており、これが検索対象画像の`.txt`サイドカーが無い場合のフォールバックである`_find_vault_category_root()`（ponyxlWildcardsVault形式——「直下にthumbnails(_option2)フォルダを持つ祖先ディレクトリ」というゆるい条件で判定）によって誤ってVaultのcategory_rootと認識されていた。誤検出されたcategory_root（1万枚超のファイルを含む出力ルート）に対し、`_load_vault_leaves()`がキャッシュの妥当性確認のため画像1枚ごとに`glob("*.yaml")`+`glob("*.yml")`を実行しており、1回あたり約53msかかるglobを53回（検索対象の画像数分）実行して合計2.8秒——実測ベンチマークで再現・確認した数値と実機ログの数値がほぼ一致した。`_load_vault_leaves()`をTTL（60秒）ベースのキャッシュに変更し、TTL内はglobを省略するようにして解消した。

**検証**: Kaptureで実ブラウザ（ComfyUI_5上のWorkflow Studio）を操作し、ネットワークリクエストのタイミングを実測しながら段階的に検証した。Python変更後の再起動が確実に反映されているかはプロセス起動時刻とファイル更新時刻を比較して都度確認した。最終的に53枚のフォルダへの検索は3327ms→74msまで短縮し、検索・クリアとも実機で即座に反映されることを確認した。デバッグ用の計測ログは検証完了後に削除済み。

**How to apply**: サブフォルダを持つ画像出力ルートで「Vault風フォルダ名（thumbnails等）の存在だけで判定するフォールバック検出」を書くと、無関係な巨大フォルダを誤検出してglob/scandir系の操作が本来の対象より何桁も遅くなることがある——検出ロジック自体だけでなく、誤検出時に何度も高コストな操作（特に大量ファイルを含むディレクトリへのglob）が繰り返し呼ばれないか、呼び出し頻度とキャッシュの妥当性も含めて確認する。原因不明のパフォーマンス低下は仮説で止めず、実機に一時的な計測ログ（`perf_counter`で区間ごとの時間を`logger.warning`に出力）を仕込んで実測すると、コードレビューだけでは気づけない誤検出のような盲点が見つかることがある。

関連: [[feedback_asyncio_blocking_in_aiohttp_routes]], [[feedback_realtime_debug_logs_and_kapture]], [[feedback_verify_restart_via_process_creation_time]]
