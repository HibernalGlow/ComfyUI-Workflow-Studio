"""
洛茜 (Rossi) 批量出图 —— 配置全部来自 storyboard 目录旁的 batch.toml

用法：
    python3.11 run_rossi_batch.py [起始序号] [--only RF003,RG001] [--tag 后缀]

需要 Python 3.11+（用标准库 tomllib 解析 TOML）。
3.9 / 3.10 上可先 `python3 -m pip install --user tomli` 再运行。

配置改在作品目录里，不再需要动代码：
    Workflows/wild/storyboard/明日方舟终末地_洛茜/batch.toml
"""
