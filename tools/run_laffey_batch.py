"""
拉菲II (Laffey II) 批量出图 —— 配置全部来自 storyboard 目录旁的 batch.toml

用法：
    tools/run_batch.sh run_laffey_batch.py [起始序号] [--only LF008,LF021] [--tag 后缀]

需要 Python 3.11+（标准库 tomllib 解析 TOML）。
3.9 / 3.10 上可先 `python3 -m pip install --user tomli` 再运行。

配置改在作品目录里，不再需要动代码：
    Workflows/wild/storyboard/碧蓝航线_拉菲II/batch.toml
"""
