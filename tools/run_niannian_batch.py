"""
念念 (Niannian) 批量出图 —— 配置全部来自 storyboard 目录旁的 batch.toml

用法：
    tools/run_batch.sh run_niannian_batch.py [起始序号] [--only NN022,NN023]

需要 Python 3.11+（标准库 tomllib 解析 TOML）。
配置改在作品目录里，不再需要动代码：
    Workflows/wild/storyboard/念念/batch.toml
"""
...
TOML = Path("/Users/glow/Base/Works/ComfyUI/Workflows/wild/storyboard/念念/batch.toml")
